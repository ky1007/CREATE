...
var textTyper = function textTyper(event) {
  ...
  var poetryTyper = function poetryTyper() {
    ...
    for (var i = 0; i < poems[poemList[0]].length; i++) {
      var _ret = _loop(i);
    ...
    }, poems[poemList[0]].length * drawTime);
  };

  if (poet === "Me! (I want to write my own poem)") {
    (function () {
      var instructions = "type out your own poem here";
...
var buttonListener = exports.buttonListener = function buttonListener() {
  ...
  button.addEventListener('click', function (event) {
    return event.target.name === undefined ? null : textTyper(event);
  });
};
